...
class CategoryController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->type == 0) {
            return redirect(route('category.index') . '?type=menu')->with('toast_success', 'Category Created Successfully!');
        } else {
            return redirect(route('category.index') . '?type=blog')->with('toast_success', 'Category Created Successfully!');
        }
    }
