<header class="header clearfix element_to_stick">
    ...
    <div class="container-fluid">
        <div id="logo">
            <a href="{{ route('home') }}">
                <img src="{{ asset($general ? $general->logo : '') }}" width="140" height="35" alt=""
                     class="logo_normal">
                <img src="{{ asset($general ? $general->logo : '') }}" width="140" height="35" alt=""
                     class="logo_sticky">
            </a>
            ...
            @else
                <li>
                    <div class="dropdown dropdown-cart">
                        <a href="{{ route('getcart', auth()->user()->id) }}" class="cart_bt"><i
                                class="fas fa-cart-plus"></i>
                            <strong>{{ \App\Models\Cart::where('user_id', auth()->user()->id)->sum('quantity') }}</strong></a>
                        <div class="dropdown-menu">
                            <ul>
                                ...
                                foreach ($carts as $cart) {
                                    ...
                                    ?>
                                <li>
                                    <figure><img src="{{ asset($cart->menu->thumbnail) }}"
                                                 data-src="{{ asset($cart->menu->thumbnail) }}" alt="" width="50"
                                                 height="50"
                                                 class="lazy"></figure>
                                    <strong><span>{{ $cart->quantity }}x {{ $cart->menu->title }}</span>${{ $cart->menu->price }}
                                    </strong>
                                        <a class="action" type="submit" href="{{ route('cart.delete', [$cart->id, auth()->user()->id]) }}"><i
                            ...
                            <div class="total_drop">
                                ...
                                <a href="{{ route('getcart', auth()->user()->id) }}" class="btn_1 outline btn_cart">View
                                    Cart</a>
                                <a href="{{ route('checkout', auth()->user()->id) }}"
                                   class="btn_1 btn_cart">Checkout</a>
                            </div>
                        </div>
                    </div>
                </li>
        ...
        <nav class="main-menu">
            ...
            <ul>
                ...
                @guest
                    <li><a href="javascript:void(0)" class="btn_top" data-bs-toggle="modal"
